...
const onClicked = async({ menuItemId, linkUrl, pageUrl, srcUrl, selectionText, checked }, tab) => {
  let content = '';
  let source = '';
...
const openPopup = async (url, closeOnFocusChange = true) => {
  ...
  if (closeOnFocusChange) {
    setTimeout(() => { winIds.add(win.id) }, 100)
  }
}
...
const onActionPressed = async (tab, src) => {
  if (typeof src != 'string') {
    ...
    try {
      ...
      if (currWinSrc) {
        src += `?source=${encodeURIComponent(currWinSrc)}`;
      }
    } catch (e) {}
  }
  const use_sidebar = await getSidebarStatus();
