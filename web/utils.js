...
window.getSelectionText = async (browser_type) => {
  const tabs = await queryCurrentTab(null, browser_type);
  const selectionText = await sendMessage(tabs[0].id, {
    msg: "get-selection-text",
  }, browser_type);
...
async function modifyUrl(url, browser_type) {
  ...
  if (url.match(ytRegex)) {
    ...
    if (tabs.length > 0) {
      const timestampData = await sendMessage(tabs[0].id, {
        msg: "obtain-timestamp",
      }, browser_type);
      url =
        `https://www.youtube.com/watch?v=${timestampData.videoId}&t=${timestampData.timestamp}`;
    }
  }
  return url;
}
